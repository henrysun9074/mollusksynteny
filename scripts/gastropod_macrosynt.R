...
# Run in bash first:
# cd /work/hs325/mollusk_synteny

# {
#   head -n 1 orthofinder/Results_May14/Orthogroups/Orthogroups.tsv
#   grep -Ff orthofinder/Results_May14/Orthogroups/Orthogroups_SingleCopyOrthologues.txt \
#     orthofinder/Results_May14/Orthogroups/Orthogroups.tsv
# } > orthofinder/Results_May14/Orthogroups/Single_copy_orthologs.tsv
...
write.table(
  ortho_ready,
  "orthofinder/Results_May14/Orthogroups/temp_ortho_for_macrosyntR.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE
...
my_orthologs_table <- load_orthologs(
  orthologs_table = "orthofinder/Results_May14/Orthogroups/temp_ortho_for_macrosyntR.tsv",
...
my_orthologs_cleaned <- my_orthologs_table %>%
  ...
  # sp3: Haliotis — NC_090280.1-NC_090295.1 = chr 1-16, remove NW_
  filter(!str_detect(sp3.Chr, "^NW")) %>%
  mutate(sp3.Chr = case_when(
    sp3.Chr == "NC_090280.1" ~ "1",
    sp3.Chr == "NC_090281.1" ~ "2",
    sp3.Chr == "NC_090282.1" ~ "3",
    sp3.Chr == "NC_090283.1" ~ "4",
    sp3.Chr == "NC_090284.1" ~ "5",
    sp3.Chr == "NC_090285.1" ~ "6",
    sp3.Chr == "NC_090286.1" ~ "7",
    sp3.Chr == "NC_090287.1" ~ "8",
    sp3.Chr == "NC_090288.1" ~ "9",
    sp3.Chr == "NC_090289.1" ~ "10",
    sp3.Chr == "NC_090290.1" ~ "11",
    sp3.Chr == "NC_090291.1" ~ "12",
    sp3.Chr == "NC_090292.1" ~ "13",
    sp3.Chr == "NC_090293.1" ~ "14",
    sp3.Chr == "NC_090294.1" ~ "15",
    sp3.Chr == "NC_090295.1" ~ "16",
    TRUE ~ sp3.Chr
  )) %>%
  ...
  # sp5: Littorina — NC_090245.1-NC_090261.1 = chr 1-17, remove NW_ and mitochondrial NC_030595.1
  filter(!str_detect(sp5.Chr, "^NW")) %>%
  filter(sp5.Chr != "NC_030595.1") %>%
  mutate(sp5.Chr = case_when(
    sp5.Chr == "NC_090245.1" ~ "1",
    sp5.Chr == "NC_090246.1" ~ "2",
    sp5.Chr == "NC_090247.1" ~ "3",
    sp5.Chr == "NC_090248.1" ~ "4",
    sp5.Chr == "NC_090249.1" ~ "5",
    sp5.Chr == "NC_090250.1" ~ "6",
    sp5.Chr == "NC_090251.1" ~ "7",
    sp5.Chr == "NC_090252.1" ~ "8",
    sp5.Chr == "NC_090253.1" ~ "9",
    sp5.Chr == "NC_090254.1" ~ "10",
    sp5.Chr == "NC_090255.1" ~ "11",
    sp5.Chr == "NC_090256.1" ~ "12",
    sp5.Chr == "NC_090257.1" ~ "13",
    sp5.Chr == "NC_090258.1" ~ "14",
    sp5.Chr == "NC_090259.1" ~ "15",
    sp5.Chr == "NC_090260.1" ~ "16",
    sp5.Chr == "NC_090261.1" ~ "17",
    TRUE ~ sp5.Chr
  )) %>%

  # sp6: Berghia — NC_088360.1-NC_088374.1 = chr 1-15, remove NW_
  filter(!str_detect(sp6.Chr, "^NW")) %>%
  mutate(sp6.Chr = case_when(
    sp6.Chr == "NC_088360.1" ~ "1",
    sp6.Chr == "NC_088361.1" ~ "2",
    sp6.Chr == "NC_088362.1" ~ "3",
    sp6.Chr == "NC_088363.1" ~ "4",
    sp6.Chr == "NC_088364.1" ~ "5",
    sp6.Chr == "NC_088365.1" ~ "6",
    sp6.Chr == "NC_088366.1" ~ "7",
    sp6.Chr == "NC_088367.1" ~ "8",
    sp6.Chr == "NC_088368.1" ~ "9",
    sp6.Chr == "NC_088369.1" ~ "10",
    sp6.Chr == "NC_088370.1" ~ "11",
    sp6.Chr == "NC_088371.1" ~ "12",
    sp6.Chr == "NC_088372.1" ~ "13",
    sp6.Chr == "NC_088373.1" ~ "14",
    sp6.Chr == "NC_088374.1" ~ "15",
    TRUE ~ sp6.Chr
  )) %>%
...
my_orthologs_cleaned <- my_orthologs_cleaned %>%
  mutate(
    ...
    sp3.Chr = factor(sp3.Chr, levels = as.character(1:16)),
    sp4.Chr = factor(sp4.Chr, levels = as.character(1:15)),
    sp5.Chr = factor(sp5.Chr, levels = as.character(1:17)),
    sp6.Chr = factor(sp6.Chr, levels = as.character(1:15)),
    sp7.Chr = factor(sp7.Chr, levels = as.character(1:19))
  )

# ── 6. Chord / ribbon diagram ─────────────────────────────────────────────────
my_labels <- c("Common Limpet", "Deep Sea Limpet", "Abalone", "Deep Sea Snail",
                "Littorinid Snail", "Nudibranch", "Scallop")
...
jpeg("results/figs/gastropod_ribbon.jpg", width = 10, height = 6, 
     units = "in", res = 300)
print(ribbon)
dev.off()